    ...
    while (!g_win.shouldClose()) {
        ...
        if (g_reload_shader){
            ...
            try {
                ...
                newProgram = loadShaders(g_file_vertex_shader, g_file_fragment_shader, g_task_chosen, g_lighting_toggle);
                g_error_message = "";
            }
            catch (std::logic_error& e) {
                //std::cerr << e.what() << std::endl;
        ...
        glUniform1f(glGetUniformLocation(g_volume_program, "iso_value_2"), g_iso_value_2);
